...
class CountIntervals {
    ...
    public void add(int left, int right) {
        ...
        if (floorEntry != null && floorEntry.getValue() >= left) {
            count -= floorEntry.getValue() - floorEntry.getKey() + 1;
            left = floorEntry.getKey();
            right = Math.max(right, floorEntry.getValue());
            map.remove(floorEntry.getKey());
        }
        while (true) {
            Map.Entry<Integer, Integer> ceilEntry = map.ceilingEntry(left);
            if (ceilEntry == null || ceilEntry.getKey() > right) {
                break;
            }
            count -= ceilEntry.getValue() - ceilEntry.getKey() + 1;
            right = Math.max(right, ceilEntry.getValue());
