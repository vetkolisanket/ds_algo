...
class NumMatrix {
    ...
    public NumMatrix(int[][] matrix) {
        ...
        for (int i=0;i<m;i++) {
            for (int j=0;j<n;j++) {
                ...
                else sumMatrix[i][j] = sumMatrix[i-1][j] + sumMatrix[i][j-1] - sumMatrix[i-1][j-1] + matrix[i][j];
            }
        }
    }
    ...
    public int sumRegion(int row1, int col1, int row2, int col2) {
        ...
        int sum2 = row1 == 0 ? 0 : sumMatrix[row1-1][col2];
        int sum3 = col1 == 0 ? 0 : sumMatrix[row2][col1-1];
        int sum4 = row1 == 0 || col1 == 0 ? 0 : sumMatrix[row1-1][col1-1];
...
//A better soln O(1) time and O(mn) space complexity
class NumMatrix {
...
//My soln O(m) time and O(mn) space complexity
class NumMatrix {
