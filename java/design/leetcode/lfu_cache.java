...
class LFUCache {
    ...
    private int minF, capacity;

    private void insert(int key, int value, int frequency) {
        Pair<Integer, Integer> pair = new Pair(frequency, value);
        cache.put(key, pair);
    ...
    public int get(int key) {
        ...
        frequencies.get(frequency).remove(key);
        if (minF == frequency && frequencies.get(frequency).isEmpty()) {
            minF++;
        }
    ...
    public void put(int key, int value) {
        ...
        if (capacity == cache.size()) {
            LinkedHashSet<Integer> keys = frequencies.get(minF);
