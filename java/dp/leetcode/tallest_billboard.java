...
//Soln using DP TC O(N*M) SC O(M) where N is the no. of rods and M is the maximum sum of the rods
class Solution {
    public int tallestBillboard(int[] rods) {
        ...
        for (int rod: rods) {
            ...
            for (Map.Entry<Integer, Integer> entry: dp.entrySet()) {
                ...
                newTaller = newDp.getOrDefault(newDiff, 0);
                newDp.put(newDiff, Math.max(newTaller, Math.max(shorter + rod, taller)));
            }
            dp = newDp;
        }
