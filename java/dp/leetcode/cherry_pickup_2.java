...
//Soln using top down DP TC O(MN^2) SC O(MN^2)
class Solution {
    ...
    private int dp(int[][] grid, int row, int col1, int col2, int[][][] memo) {
        if (row == grid.length || col1 == -1 || col1 == grid[0].length || 
        col2 == -1 || col2 == grid[0].length) {
            return 0;
        }
        ...
        for (int i=col1-1;i<=col1+1;i++) {
            for (int j=col2-1;j<=col2+1;j++) {
                max = Math.max(max, dp(grid, row+1, i, j, memo));
            }
        }
        return memo[row][col1][col2] = sum + max;
    }
}
